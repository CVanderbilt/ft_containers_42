...
VectorTesterStd& VectorTesterStd::withIterators() { method = RANGE; return *this; }
VectorTesterStd& VectorTesterStd::withHeaders() {  headers = true; return *this; }
VectorTesterStd& VectorTesterStd::withStop() {  _stop = true; return *this; }
VectorTesterStd& VectorTesterStd::withCopy() { method = COPY; return *this; }
VectorTesterStd& VectorTesterStd::withSize(size_t s) {
	size = s;
	if (size < 10) {
...
void VectorTesterStd::build() { construct_map(); }

void VectorTesterStd::execute(const std::string& testName) {
...
void VectorTesterStd::error(std::string str) { __out << "[ERROR]: " << str << std::endl; }
void VectorTesterStd::printMap() { 
	for (iterator it = cnt->begin(); it != cnt->end(); it++) __out << *it; __out << std::endl;
}
void VectorTesterStd::printHeader(std::string testName) { if (headers) __out << "[TEST]: " << testName << std::endl; }
int VectorTesterStd::generateNumber(size_t mod = 10000) const { int r = rand(); r *= r < 0 ? -1 : 1; return r % mod; }
...
void VectorTesterStd::construct_map() {
	if (cnt != NULL) return ;
	switch (method)
	{
	case COPY:
...
void VectorTesterStd::construct_default() {
...
void VectorTesterStd::construct_iterator() {
...
void VectorTesterStd::construct_copy() {
...
void VectorTesterStd::test_equal_operator() {
...
void VectorTesterStd::test_iterators() {
...
void VectorTesterStd::test_capacity() {
	t_vector emptyMap;
...
void VectorTesterStd::test_element_access() {
...
void VectorTesterStd::test_erase() {
...
void VectorTesterStd::test_swap() {
...
void VectorTesterStd::test_insert() {
...
void VectorTesterStd::test_relational() {
