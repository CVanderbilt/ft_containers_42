...
MapTesterStd& MapTesterStd::withIterators() { method = RANGE; return *this; }
MapTesterStd& MapTesterStd::withHeaders() {  headers = true; return *this; }
MapTesterStd& MapTesterStd::withStop() {  _stop = true; return *this; }
MapTesterStd& MapTesterStd::withCopy() { method = COPY; return *this; }
MapTesterStd& MapTesterStd::withSize(size_t s) {
	size = s;
	if (size < 10) {
...
void MapTesterStd::build() { construct_map(); }

void MapTesterStd::execute(const std::string& testName) {
...
void MapTesterStd::error(std::string str) { __out << "[ERROR]: " << str << std::endl; }
void MapTesterStd::printMap() { 
	for (iterator it = cnt->begin(); it != cnt->end(); it++) __out << it->first << it->second; __out << std::endl;
}
void MapTesterStd::printHeader(std::string testName) { if (headers) __out << "[TEST]: " << testName << std::endl; }
int MapTesterStd::generateNumber(size_t mod = 10000) const { int r = rand(); r *= r < 0 ? -1 : 1; return r % mod; }
...
void MapTesterStd::construct_map() {
	if (cnt != NULL) return ;
	switch (method)
	{
	case COPY:
...
void MapTesterStd::construct_map_default() {
...
void MapTesterStd::construct_map_iterator() {
...
void MapTesterStd::construct_map_copy() {
...
void MapTesterStd::test_equal_operator() {
...
void MapTesterStd::test_iterators() {
...
void MapTesterStd::test_capacity() {
	t_map emptyMap;
...
void MapTesterStd::test_element_access() {
...
void MapTesterStd::test_hinted_insert() {
...
void MapTesterStd::test_relational() {
