...
public:
	...
	_red_black_tree(): r(NULL), _cmp(), _size(0), _alloc(), _valloc() {}
	_red_black_tree(const _red_black_tree& t);
	~_red_black_tree() {}

	_red_black_tree& operator= (const _red_black_tree& rhs);
	...
	iterator get(const T target) { t_node *node = getNode(target); return iterator(node, r); }
